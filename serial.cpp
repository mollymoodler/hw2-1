...
/**
 * Simulates one step of the particle system (sequential).
 */
void simulate_one_step(particle_t* parts, int num_parts, double size, int bin_count, double bin_size, 
    ...
    // Step 2: Move particles and update bins
    ...
    for (int i = 0; i < num_parts; i++) {
        ...
        int new_bin = get_bin_index(parts[i].x, parts[i].y, bin_count, bin_size);
