...
void init_simulation(particle_t* parts, int num_parts, double size) {
    ...
    // Precompute valid neighbor bins for each bin
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    std::unordered_set<int> cross_boundary_particles; // Store indices of particles that moved across bins

    // Step 1: Compute forces using precomputed neighbors & avoiding redundancy
    for (int bx = 0; bx < bin_count; bx++) {
        for (int by = 0; by < bin_count; by++) {
            int bin_index = bx * bin_count + by;
    ...
    // Step 2: Move particles and update bin assignments **only for crossing particles**
    for (int i = 0; i < num_parts; i++) {
        int old_bin;
        if (move(parts[i], size, old_bin)) { // If the particle moved to a different bin
            cross_boundary_particles.insert(i);
        }
    }

    // Step 3: Only update bins for cross-boundary particles
    for (int i : cross_boundary_particles) {
        int old_bin = get_bin_index(parts[i].x, parts[i].y);
        
        // Remove from old bin
        auto& bin_list = bins[old_bin];
        bin_list.erase(std::remove(bin_list.begin(), bin_list.end(), i), bin_list.end());
